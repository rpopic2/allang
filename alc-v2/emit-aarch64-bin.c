...
/* ELF executable arch hooks (see emit-elf-exe.h) */

const uint16_t elf_machine = 183;            /* EM_AARCH64 */
const uint32_t elf_reloc_jump_slot = 1026;   /* R_AARCH64_JUMP_SLOT */
const char *elf_interp = "/lib/ld-linux-aarch64.so.1";

const uint32_t elf_start_stub_size = 12;
const uint32_t elf_dyn_start_stub_size = 8;
const uint32_t elf_plt0_size = 32;
const uint32_t elf_plt_entry_size = 16;

static void put_le32(uint8_t *p, uint32_t w) {
    memcpy(p, &w, 4);
}
...
static uint32_t enc_ldr64(uint32_t rt, uint32_t rn, uint64_t target) {
    uint32_t imm12 = (uint32_t)((target & 0xFFFu) >> 3);
    return 0xF9400000u | (imm12 << 10) | (rn << 5) | rt;
}

static uint32_t enc_add64(uint32_t rd, uint32_t rn, uint64_t target) {
    uint32_t imm12 = (uint32_t)(target & 0xFFFu);
    return 0x91000000u | (imm12 << 10) | (rn << 5) | rd;
}

/* static _start: bl main; mov w8, #93; svc #0   (w0 = main's return = exit code) */
...
/* dynamic _start: bl main; bl exit@plt   (w0 carries the exit code into exit) */
...
/* PLT[0] lazy resolver trampoline (loads &GOT[2] = ld.so resolver). */
void elf_build_plt0(uint8_t *p, uint64_t plt0_vaddr, uint64_t got_vaddr) {
    uint64_t g2 = got_vaddr + 16;
    put_le32(p + 0,  0xA9BF7BF0u);                     /* stp x16, x30, [sp, #-16]! */
    put_le32(p + 4,  enc_adrp(16, plt0_vaddr + 4, g2));
    put_le32(p + 8,  enc_ldr64(17, 16, g2));
    put_le32(p + 12, enc_add64(16, 16, g2));
    put_le32(p + 16, 0xD61F0220u);                     /* br x17 */
    put_le32(p + 20, 0xD503201Fu);                     /* nop */
    put_le32(p + 24, 0xD503201Fu);
    put_le32(p + 28, 0xD503201Fu);
}

/* PLT[n]: adrp/ldr/add &GOT[n]; br x17. GOT entry starts at PLT[0] so the
   first call routes through the lazy resolver. */
uint64_t elf_build_plt_entry(uint8_t *p, uint32_t index, uint64_t plt_vaddr,
    ...
    put_le32(p + 12, 0xD61F0220u);                     /* br x17 */
    return plt0_vaddr;
}
...
void elf_patch_call(uint8_t *site, uint64_t site_vaddr, uint64_t target_vaddr) {
    uint32_t w;
    memcpy(&w, site, 4);
    w = (w & ~0x03FFFFFFu) | ((uint32_t)(((int64_t)target_vaddr - (int64_t)site_vaddr) >> 2) & 0x03FFFFFFu);
    memcpy(site, &w, 4);
}
