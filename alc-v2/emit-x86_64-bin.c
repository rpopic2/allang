...
const uint16_t elf_machine = 62;             /* EM_X86_64 */
const uint32_t elf_reloc_jump_slot = 7;      /* R_X86_64_JUMP_SLOT */
const char *elf_interp = "/lib64/ld-linux-x86-64.so.2";

const uint32_t elf_start_stub_size = 14;
const uint32_t elf_dyn_start_stub_size = 12;
const uint32_t elf_plt0_size = 16;
...
void elf_build_start_stub(uint8_t *p, uint64_t stub_vaddr, uint64_t main_vaddr) {
    int32_t call_rel = (int32_t)((int64_t)main_vaddr - (int64_t)(stub_vaddr + 5));
    p[0] = 0xe8;
    memcpy(p + 1, &call_rel, 4);
    p[5] = 0x89;
    p[6] = 0xc7;
    p[7] = 0xb8;
    p[8] = 60;
...
void elf_build_dyn_start_stub(uint8_t *p, uint64_t stub_vaddr,
    ...
    int32_t call_main = (int32_t)((int64_t)main_vaddr - (int64_t)(stub_vaddr + 5));
    int32_t call_exit = (int32_t)((int64_t)exit_plt_vaddr - (int64_t)(stub_vaddr + 12));
    p[0] = 0xe8;
    memcpy(p + 1, &call_main, 4);
    p[5] = 0x89;
    p[6] = 0xc7;
    p[7] = 0xe8;
    memcpy(p + 8, &call_exit, 4);
}

/* PLT[0]: push QWORD PTR [rip+GOT[1]]; jmp QWORD PTR [rip+GOT[2]]; nop*4 */
...
/* PLT[n]: jmp [rip+GOT[n]]; push index; jmp PLT[0]. The GOT entry starts out
   pointing at the push so the first call falls through to the lazy resolver. */
