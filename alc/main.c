    ...
    while (true) {
        ...
        if (slice && str_eq_lit(mem_name, "Length")) {
            ...
            if (result.reg.reg_type != STACK) {
                result.reg.offset += member_storage.offset;
            }
            break;
        }
...
dyn_agg_member *read_aggregate(allocator *alloc, parser_context *context, dtype_t *dtype) {
    const token_t *token = &context->cur_token;
...
void dyn_slice_access(parser_context *context, const reg_t *lhs, i32 len) {
    ...
    bool is_range = false;
    for (const char *p = rhs_token.id.data; p + 1 < rhs_token.id.end; ++p) {
        if (p[0] == '.' && p[1] == '.') {
            is_range = true;
            break;
        }
    ...
    const regable length = is_slice
        ? (regable){.tag = REG, .reg = count_reg}
...
void expr_struct(parser_context *context, reg_t target, dtype_t *dtype) {
    ...
    dyn_agg_member *args = read_aggregate(&alloc, context, dtype);
    ...
    if (lhs.tag == VALUE) {
        ...
        if (reg_type == NREG) {
            ...
            if (dtype_outer(&nreg->dtype).tag == DK_SLICE) {
                ...
                reg_t dst_len = context->reg;
                dst_len.offset += 1;
                dst_len.rsize = sizeof (void *);
                emit_mov_reg(dst_len, src_len);
            }
...
void expr_cmp(const reg_t *lhs, const regable* rhs, cond_t cond_optimize) {
    if (rhs->tag == REG) {
        emit_cmp_reg(*lhs, rhs->reg, cond_optimize);
    } else {
        emit_cmp(*lhs, rhs->value);
    }
}
